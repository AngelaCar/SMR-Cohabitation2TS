# ---- Figure S11 ----
# Plot of SEs for the separation model and comparison with sample first cohabitation only 
...
load("R_int2d_separation_we.Rda")
load("R_int2d_separation_ww.Rda")
...
p_SE_full_me <- get_hazard_2d(r_sep_nocov_18p_me$mod_no_cov,
                              plot_grid = list(
                                c(18, 47, .2),
                                c(0, 26, .2)
                              ))

p_SE_full_mw <- get_hazard_2d(r_sep_nocov_18p_mw$mod_no_cov,
                              plot_grid = list(
                                c(18, 47, .2),
                                c(0, 25, .2)
                              ))

p_SE_full_we <- get_hazard_2d(r_sep_nocov_18p_we$mod_no_cov,
                              plot_grid = list(
                                c(18, 47, .2),
                                c(0, 25, .2)
                              ))

p_SE_full_ww <- get_hazard_2d(r_sep_nocov_18p_ww$mod_no_cov,
                              plot_grid = list(
                                c(18, 47, .2),
                                c(0, 27, .2)
                              ))

p_SE_frst_me <- get_hazard_2d(r_sep_nocov_frst_me$mod_no_cov,
                              plot_grid = list(
                                c(18, 46, .2),
                                c(0, 26, .2)
                              ))

p_SE_frst_mw <- get_hazard_2d(r_sep_nocov_frst_mw$mod_no_cov,
                              plot_grid = list(
                                c(18, 47, .2),
                                c(0, 25, .2)
                              ))

p_SE_frst_we <- get_hazard_2d(r_sep_nocov_frst_we$mod_no_cov,
                              plot_grid = list(
                                c(18, 43, .2),
                                c(0, 25, .2)
                              ))

p_SE_frst_ww <- get_hazard_2d(r_sep_nocov_frst_ww$mod_no_cov,
                              plot_grid = list(
                                c(18, 44, .2),
                                c(0, 27, .2)
                              ))
